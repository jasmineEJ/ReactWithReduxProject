...
const router = jsonServer.router(path.join(__dirname, "airlines_db.json"));
...
server.post("/app/flights/", function(req, res, next) {
  const error = validateFlights(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.flightId); // Generate a slug for new courses.
    next();
  }
});

server.post("/app/passengers/", function(req, res, next) {
  const error = validatePassenger(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.firstName = createSlug(req.body.firstName);
    next();
  }
});

server.post("/app/ancillary/", function(req, res, next) {
  const error = validateService(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.serviceName = createSlug(req.body.serviceName);
    next();
  }
});
...
function validateFlights(flight) {
  if (!flight.flightName) return "Flight Name is required.";
  return "";
}
function validatePassenger(passenger) {
  if (!passenger.firstName) return "Passenger Name is required.";
  return "";
}

function validateService(service) {
  if (!service.serviceName) return "Service Name is required";
  return "";
}
